...
module.exports = {
  ...
  overviewProduct: async (req, res) => {
    try {
      ...
      const resultat = {
        ...
        CATEGORIE: productInformation.CATEGORIE
          ? productInformation.CATEGORIE.NOM_CATEGORIE
          : null,
      };
  ...
  getSupplierDetails: async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          "Erreur lors de la récupération du fournisseur: " + error.message,
      });
    }
  },
  ...
  getStockLocations: async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          "Erreur lors de la récupération de l'emplacement: " + error.message,
      });
    }
  },
    ...
    try {
      ...
      salesData.forEach((sale) => {
        ...
        if (period === "day") {
          ...
          displayName = `${sale["Jour"]} ${getMonthName(sale["Mois"])} ${
            sale["Année"]
          }`;
        } else if (period === "week") {
          ...
          displayName = `Semaine du ${firstDayOfWeek.toLocaleDateString()} - Semaine ${
            sale["Semaine"]
          } de l'année ${sale["Année"]}`;
      ...
      purchaseData.forEach((purchase) => {
        ...
        if (period === "day") {
          ...
          displayName = `${purchase["Jour"]} ${getMonthName(
            purchase["Mois"]
          )} ${purchase["Année"]}`;
        } else if (period === "week") {
      ...
      console.error(
        "Erreur lors de la récupération des ventes et des achats par période :",
        error
      );
      res.status(500).json({
        message:
    ...
    try {
      ...
      for (const obj in mergedData) {
        if (mergedData[obj].CommandesClient && mergedData[obj].VentesMagasin)
          mergedData[obj].TotalRevenus =
            mergedData[obj].CommandesClient + mergedData[obj].VentesMagasin;
      }
  ...
  productQuantityHistory: async (req, res) => {
    try {
      ...
      const revenus = await models.stock.findAll({
        attributes: [
          ...
          [
            sequelize.fn(
              "ROUND",
              sequelize.fn("AVG", sequelize.col("QUANTITE_STOCK"))
            ),
            "averageQuantity",
          ],
        ],
      ...
      console.error(
        "Erreur lors de la récupération des quantités et de revenus par période :",
        error
      );
      res.status(500).json({
        message:
          "Erreur lors de la récupération des quantités et des revenus par période.",
      });
    }
  },
  ...
  addOrSustractProduit: async (req, res) => {
    ...
    try {
      ...
      const incrementResult = await product.increment("QUANTITE", {
        by: quantity,
      });
      ...
      console.error(
        "Erreur lors de l incrémentation ou décrementation de la quantité :",
        error
      );
      res.status(500).json({
        message:
